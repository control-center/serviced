...
func (s *DFSTestSuite) TestRollback_Success(c *C) {
	...
	vol.On("SnapshotInfo", "BASE_LABEL").Return(vinfo, nil)
	vol.On("ReadMetadata", "LABEL", ImagesMetadataFile).Return(&NopCloser{vimagesbuf}, nil)
	vol.On("Name").Return("BASE_LABEL")
	vol.On("Path").Return("TEST_PATH")
	s.disk.On("GetTenant", "").Return(vol, nil)
	...
	s.net.On("RemoveVolume", "TEST_PATH").Return(nil)
	s.net.On("AddVolume", "TEST_PATH").Return(nil)

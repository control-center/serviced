...
func (s *DFSTestSuite) TestCommit_NoPush(c *C) {
	...
	s.index.On("PushImageAfterCommit", "libraryname/reponame:latest", "testimage2").Return(ErrTestNoPush)
	s.index.On("PushImage", "libraryname/reponame:latest", "testimage").Return(nil)
	s.docker.On("RemoveImage", "testimage2").Return(nil)
	tenantID, err := s.dfs.Commit("testcontainer")
	c.Assert(tenantID, Equals, "")
...
func (s *DFSTestSuite) TestCommit_Success(c *C) {
	...
	s.index.On("FindImage", "localhost:5000/libraryname/reponame:latest").Return(rImg, nil)
	s.docker.On("CommitContainer", "testcontainer", "localhost:5000/libraryname/reponame:latest").Return(img, nil)
	s.index.On("PushImageAfterCommit", "localhost:5000/libraryname/reponame:latest", "testimage2").Return(nil)
	s.index.On("PushImageAfterCommit", "libraryname/reponame:latest", "testimage2").Return(nil)
	tenantID, err := s.dfs.Commit("testcontainer")

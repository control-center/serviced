...
func init() {
	...
	elasticsearch_serviced, err = NewContainer(
		ContainerDescription{
			...
			Repo:        IMAGE_REPO,
			Tag:         IMAGE_TAG,
			Command:     func () string {return fmt.Sprintf(`/opt/elasticsearch-0.90.9/bin/elasticsearch -f -Des.node.name=%s`, serviceName)},
			Ports:       []int{9200},
	...
	elasticsearch_logstash, err = NewContainer(
		ContainerDescription{
			...
			Repo:        IMAGE_REPO,
			Tag:         IMAGE_TAG,
			Command:     func() string {return fmt.Sprintf(`/opt/elasticsearch-1.3.1/bin/elasticsearch -Des.node.name=%s`, serviceName)},
			Ports:       []int{9100},

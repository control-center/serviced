...
Given (/^(?:|that )the "([^"]*)" port does not exist$/) do |name|
    ...
    CC.UI.login()
    CC.UI.ServicesPage.remove_publicendpoint_port_json(name) if CC.CLI.service.check_publicendpoint_port_exists_json(name)
end
...
Given (/^(?:|that )the port public endpoint "([^"]*)" is removed$/) do |name|
    ...
    CC.UI.login()
    CC.UI.ServicesPage.remove_publicendpoint_port_json(name)
end

...
func (a *HostAgent) attachToService(conn coordclient.Connection, procFinished chan<- int, serviceState *servicestate.ServiceState) (bool, error) {
	// get docker status
	...
	go func() {
		...
		if err != nil || rc != 0 || glog.GetVerbosity() > 0 {
			...
			if err != nil {
				...
				str = str[last:]
				glog.Warningf("Last 50000 bytes of container %s: %s", containerID, str)
...
func (a *HostAgent) processServiceState(conn coordclient.Connection, shutdown <-chan int, done chan<- stateResult, ssID string) {
	...
	for {
		...
		case ss.Started.Year() > 1 && ss.Terminated.Year() <= 1:
			...
			attached, err = a.attachToService(conn, procFinished, &ss)

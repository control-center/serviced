...
func New(driver api.API, config utils.ConfigReader, logControl logging.LogControl) *ServicedCli {
	...
	c.app.Flags = []cli.Flag{
		...
		cli.IntFlag{"es-request-timeout", defaultOps.ESRequestTimeout, "elasticsearch client connection timeout in seconds"},
		...
		cli.StringFlag{"isvcs-zk-acl-user", defaultOps.IsvcsZkAclUser, "isvcs zookeeper acl user"},
		cli.StringFlag{"isvcs-zk-acl-passwd", defaultOps.IsvcsZkAclPasswd, "isvcs zookeeper acl password"},
...
func getRuntimeOptions(cfg utils.ConfigReader, ctx *cli.Context) config.Options {
	options := config.Options{
		...
		IsvcsZkAclUser:             ctx.GlobalString("isvcs-zk-acl-user"),
		IsvcsZkAclPasswd:           ctx.GlobalString("isvcs-zk-acl-passwd"),

...
func New(driver api.API) *ServicedCli {
	...
	if len(configEnv("REMOTE_ZK", "")) > 0 {
		remoteZKs = cli.StringSlice(strings.Split(configEnv("REMOTE_ZK", ""), ","))
	}
	...
	c.app.Flags = []cli.Flag{
		...
		cli.StringSliceFlag{"remote-zk", &remoteZKs, "Specify a remote zookeeper instance to connect to across a WAN (e.g. -remote-zk remotehost:2181)"},

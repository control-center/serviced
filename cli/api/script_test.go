...
func (s *CliAPITestSuite) Test_script_cliServiceMigrate(c *C) {
	mockAPI := new(mockAPI)
	mockAPI.Mock.
...
func (s *CliAPITestSuite) Test_script_cliServiceMigrateWithVersion(c *C) {
	mockAPI := new(mockAPI)
	mockAPI.Mock.
...
func (s *CliAPITestSuite) Test_script_cliServiceMigrate_fails(c *C) {
	...
	c.Assert(err, ErrorMatches, "Migration failed for service serviceID: errorStub: migration failed")
}

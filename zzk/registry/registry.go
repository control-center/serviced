...
func GetVHosts(conn client.Connection) (map[VHostKey]VHost, error) {
	...
	for _, hostID := range hostIDs {
		...
		if err == client.ErrNoNode {
			hostLogger.Debug("Host has been deleted forvirtual hosts")
		} else if err != nil {
...
func syncServiceVHosts(conn client.Connection, tx client.Transaction, request ServiceRegistrySyncRequest) error {
	...
	for _, vhostKey := range request.VHostsToDelete {
		...
		tx.Delete(addrpth)
		addrLogger.Info("Deleted virtual host")
	}
	...
	for vhostKey, vhost := range request.VHostsToPublish {
		...
		if err == client.ErrInvalidPath {
			...
			addrLogger.Info("Created public port address")
		} else if err == client.ErrNodeExists {
			existingVHost := &VHost{}
			...
			addrLogger.Info("Updated virtual host")
		}
	}

	logger.Info("Updated transaction to sync virtual hosts for service")
	return nil
}

...
// MissingAddressAssignment is an error type for missing address assignments
type MissingAddressAssignment struct {
	ServiceID    string
	EndpointName string
}

// Error implements error
func (err MissingAddressAssignment) Error() string {
	return fmt.Sprintf("facade: service %s is missing an address assignment for endpoint %s", err.ServiceID, err.EndpointName)
}

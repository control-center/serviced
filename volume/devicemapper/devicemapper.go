...
func (d * DeviceMapperDriver) cleanUpSnapshots() {
    ...
    for snapshotOnDisk := range snapshotsOnDisk {
	...
	for snapshotInMetadata, volumeName := range snapshotsInMetadata {
		if _, ok := snapshotsOnDisk[snapshotInMetadata]; !ok {
...
func (d * DeviceMapperDriver) getSnapshotsOnDisk() (map[string]struct{}, error) {  
...
func (v *DeviceMapperVolume) Export(label, parent string, writer io.Writer) error {
	...
	defer func(d *DeviceMapperDriver, deviceHash, mountpoint string) {
		...
		if err := v.driver.DeviceSet.UnmountDevice(deviceHash); err != nil {
